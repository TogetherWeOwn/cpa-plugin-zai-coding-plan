...
func writeJSONAt(dir *os.File, name string, data []byte, syncDir func(*os.File) error) error {
	...
	if errSync := syncDir(dir); errSync != nil {
		return &WriteError{Outcome: WriteNeedsRecovery, Err: fmt.Errorf("sync secure store directory: %w", errSync)}
	}
	return nil
}
...
func removeJSONAt(dir *os.File, name string, syncDir func(*os.File) error) error {
	...
	if err := syncDir(dir); err != nil {
		return &WriteError{Outcome: WriteNeedsRecovery, Err: fmt.Errorf("sync secure store directory: %w", err)}
	}
	return nil
}
