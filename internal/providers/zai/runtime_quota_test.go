...
func TestRuntimeShutdownPersistsWarningAfterAdmittedUsageFailure(t *testing.T) {
	...
	reloadedStore, err := newSecureStore(filepath.Dir(filepath.Dir(runtime.snapshot.Store.dir)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRuntimeDifferentStoreReconfigureWaitsForAdmittedUsage(t *testing.T) {
	...
	reloadedStore, err := newSecureStore(filepath.Dir(filepath.Dir(oldStore.dir)))
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _ = reloadedStore.close() }()
