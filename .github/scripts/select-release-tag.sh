...
event_name=${1:?usage: select-release-tag.sh EVENT_NAME EVENT_REF EVENT_SHA WORKFLOW_RUN_SHA}
event_ref=${2:?usage: select-release-tag.sh EVENT_NAME EVENT_REF EVENT_SHA WORKFLOW_RUN_SHA}
event_sha=${3:?usage: select-release-tag.sh EVENT_NAME EVENT_REF EVENT_SHA WORKFLOW_RUN_SHA}
workflow_run_sha=${4-}
...
case "$event_name" in
  ...
  workflow_run)
    ...
    test "$(git rev-parse "$raw_tag")" = "$recovery_tag_object"
