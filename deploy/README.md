# v0.4.0 subscription-pool deployment
...
- The tagged v0.4.0 commit, registry bytes, and release checksums are recorded by the release artifact manifest; do not substitute a working-tree or untagged artifact.
...
```sh
bundle=/path/to/subscription-pool-v0.4.0-operator.zip
...
release_dir=/path/to/downloaded-v0.4.0-release-assets
...
expected={"id":"subscription-pool","version":"0.4.0","install_type":"github-release"}
if any(response.get(key) != value for key, value in expected.items()):
    raise SystemExit("plugin install response did not confirm the expected release")
path_value=response.get("path")
if not isinstance(path_value, str) or "/linux/amd64/" not in path_value or "0.4.0" not in path_value:
